...
# Creating directories if absent
...
# Identifying face using pretrained model
...
def add_attendance(name):
    ...
    if int(userid) not in list(df['Roll']):
        with open(f'Attendance/Attendance-{date_today}.csv', 'a') as f:
...
def home():
    ...
    return render_template('home.html', names=names, rolls=rolls, times=times, l=l, totalreg=totalreg(),
                           datetoday2=date_today2)


# Function to run when the Take Attendance Button is clicked
@app.route('/start', methods=['GET'])
def start():
    if 'face_recognition_model.pkl' not in os.listdir('static'):
        return render_template('home.html', totalreg=totalreg(), datetoday2=date_today2,
